...
**Android source:** Populated from the Mux API (VOD) or static config for StreamKit's three live sources — Red Bull TV, DW English, and NHK World-Japan. See `specs/technical/content-catalog.md` for full detail on each, including verification status. Serves as the local catalog cache.

**Backend source:** Created on ingestion (Phase 4) or managed as static records for Mux-sourced and live content.
...
- **Mux asset filtering (BR-CAT-04):** only Mux assets with `status == "ready"` are mapped into `VideoEntity` rows and written to the cache. Assets in `preparing`, `errored`, or any other status are excluded at the repository layer and must never appear in the catalog. A `"ready"` asset is *also* excluded if it has no playback ID with `policy == "public"` — Phase 1 has no signing-key infrastructure, so a signed-only (or playback-ID-less) asset is unplayable regardless of its `status`, and is excluded the same way and at the same layer as a non-ready asset. Log a warning when an asset is excluded for this reason, since it's rarer and more useful to notice than the routine non-ready case.
- **Mux title/description mapping:** Mux has no native `title`/`description` field on an asset. For Mux-sourced VOD, `title`/`description` are parsed from the asset's `passthrough` metadata (JSON, set at upload time: `{"title": "...", "description": "..."}`) — not derived from the Mux asset ID, filename, or dashboard-only metadata. If `passthrough` is missing or fails to parse, `title` falls back to the Mux asset `id` and `description` falls back to `null`. This applies to any future feature that consumes Mux assets, not just Catalog — see `specs/features/catalog/requirements.md` RQ-CAT-18/RQ-CAT-19 and `content-catalog.md`'s "VOD Source — Mux → Naming Convention" section for full detail.
...
**Notes:**
- `source` distinguishes the origin of the video record: `'MUX'` (Mux API VOD), `'LOCAL'` (ingested via the Phase 4 pipeline), or `'STATIC'` (the three live streams, seeded at startup — no schema change was needed to go from one static live entry to three; the table already supports multiple `'STATIC'`/`LIVE` rows).
...
| 3 | Should `VideoEntity` in Room include a `source` field (MUX / LOCAL / STATIC) mirroring the backend? | Not required for Phase 1 — all Phase 1 content is Mux or live static entries; add in Phase 4 if catalog mixing requires it |
| 4 | **Open — does the Red Bull TV stream URL actually play?** Manual testing of `https://rbmn-live.akamaized.net/hls/live/590964/BoRB-AT/master.m3u8` produced no video, despite the URL resolving with a valid HLS content type. Suspected cause: geo-restriction (`X-GeoBlock: true` observed on the host). | **Blocking before Phase 1 implementation.** Verify playback directly (VLC, ExoPlayer test, or a different network/region) before relying on this as a live content source. If unresolvable, evaluate alternative variant URLs (`master_1660.m3u8`, `master_3360.m3u8`) or fall back to the other two carousel sources below. |
| 5 | **Open — do the DW English and NHK World-Japan candidate URLs actually play?** Neither has been manually tested, and neither received the same HTTP/manifest-check depth that Red Bull TV's original research got — see `content-catalog.md` Live Source 2/3 for exactly what was and wasn't checked. | **Blocking before Phase 1 implementation**, same as Question #4. Also pending Dani's confirmation that these are the right two sources to add — see `CONTEXT.md` Open Question #7. |
...
| 0.1.7 | 2026-07-10 | Danielle Mariani | Added a general Mux title/description mapping note to the `Video` entity, following the BR-CAT-04 precedent: `title`/`description` are parsed from the Mux asset's `passthrough` JSON metadata, falling back to `id`/`null` only when missing or malformed. Resolves the naming-convention half of `content-catalog.md` Open Question #2. (This change was previously applied to a stale local copy that wasn't uploaded to Project files; reapplied here to the actual current version.) |
| 0.1.8 | 2026-07-11 | Danielle Mariani | Broadened BR-CAT-04's Mux-mapping note, found during TSK-CAT-13 implementation: a `"ready"` asset is now also excluded if it has no `"public"`-policy playback ID, since Phase 1 has no signing-key infrastructure and such an asset would be unplayable regardless of status. Same repository-layer exclusion pattern as the existing status filter, with a warning log on exclusion |